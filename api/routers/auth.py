...
@router.post(
    ...
    **Rate limit:** 5 requests per minute
    """,
    responses={
        201: {"description": "User successfully registered", "model": models.UserResponse},
        400: {
...
@router.post(
    ...
    **Rate limit:** 300 requests per minute
    """,
    responses={
        200: {"description": "Email successfully verified", "model": models.SuccessResponse},
        400: {
...
@router.post(
    ...
    **Rate limit:** 10 requests per minute
    """,
    responses={
        200: {"description": "Authentication successful", "model": models.Token},
        401: {
...
async def request_password_reset(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create reset token"
        )
    ...
    background_tasks.add_task(
        _send_and_cleanup, password_reset_request.email, token, user.get("language", "en")
    )
    ...
    **Rate limit:** 300 requests per minute
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {"example": {"message": "Password successfully reset"}}
            },
        },
        400: {"description": "Bad Request - Invalid or expired token", "model": models.HTTPError},
        429: {"description": "Too Many Requests - Rate limit exceeded"},
        500: {"description": "Internal Server Error"},
    },
)
...
async def confirm_password_reset(
    ...
    ok = await database.confirm_password_reset_transaction(
        pool, password_reset_confirm.token, new_password_hash
    )
    if not ok:
        raise HTTPException(
