...
async def create_user_rss_feed(_request: Request, feed: models.UserRSSFeedCreate, current_user: dict = Depends(get_current_user)):
    # Validate RSS URL
...
@limiter.limit("300/minute")
async def get_user_rss_feed(_request: Request, feed_id: int, current_user: dict = Depends(get_current_user)):
    pool = await database.get_db_pool()
    if pool is None:
        raise HTTPException(status_code=500, detail="Database error")
...
@limiter.limit("300/minute")
async def update_user_rss_feed(_request: Request, feed_id: int, feed_update: models.UserRSSFeedUpdate, current_user: dict = Depends(get_current_user)):
    # Validate input lengths
...
@limiter.limit("300/minute")
async def delete_user_rss_feed(_request: Request, feed_id: int, current_user: dict = Depends(get_current_user)):
    pool = await database.get_db_pool()
    if pool is None:
        raise HTTPException(status_code=500, detail="Database error")
